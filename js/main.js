...
var tabel   = document.getElementById('tabel-mahasiswa'),
    ...
    form.addEventListener('click',hapusBaris);
    ...
    //tambahkan bagian button delete
...
function bacaDariDB(){
    ...
    objectStore.openCursor().onsuccess = (e) => {
        ...
        if(result){
            console.log('Membaca mahasiswa[ '+result.value.nim+ '] Dari DB' );
...
function hapusBaris(e){
    if(e.target.type=='button'){
        ...
        if(hapus){
            tabel.deleteRow(table.rows.namedItem(e.target.id).selectionRowIndex);
...
function hapusDariDB(nim){
    ...
    request.oneerror = errordbHandle;
